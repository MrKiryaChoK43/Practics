async function getCurrentWeatherByCity(city) {
    const data = await fetch('http://api.weatherapi.com/v1/current.json?key=ed1fc965fab34f2ea03100100241811&q=${city}&aqi=no')
...
async function getForecastByCity(city) {
    const forecast = await fetch('http://api.weatherapi.com/v1/forecast.json?key=ed1fc965fab34f2ea03100100241811&q=${city}&days=1&aqi=no&alerts=no')
    const currentWeather = await forecast.json()
...
function createForecastElement(iconSrc, time, temperature) {
    ...
    const forecastTime = document.createElementF('p')
    forecastTime.setAttribute('class', "forecast-time")
    forecastTime.innerHTML = time.slice(11)

    const forecastIcon = document.createElementF('img')
    forecastTime.setAttribute('class', "forecast-icon")
    forecastTime.setAttribute('src', 'http:${iconSrc}')
